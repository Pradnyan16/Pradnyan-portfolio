...
export default function Tools() {
  return (
    <section id="tools" className="w-full bg-transparent text-black dark:text-white py-24 border-b border-black/10 dark:border-white/10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 w-full h-full">
            {tools.map((tool, index) => (
              <motion.div
                ...
                className="flex flex-col items-center justify-center p-8 border-b border-r border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5 transition-colors group cursor-crosshair aspect-square"
              >
                <img 
                ...
                />
                <span className="font-mono text-[10px] md:text-xs uppercase tracking-widest text-zinc-500 group-hover:text-black dark:group-hover:text-white transition-colors text-center">
                  {tool.name}
                </span>
              </motion.div>
            ))}
            ...
            {Array.from({ length: (4 - (tools.length % 4)) % 4 }).map((_, i) => (
              <div key={`empty-${i}`} className="hidden lg:block border-b border-r border-black/10 dark:border-white/10 aspect-square"></div>
            ))}
          </div>
        </div>
      </div>
